...
export function BossVictoryScreen({ palier, gemsEarned, coinsEarned, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ fontFamily: 'var(--f-ui)', fontSize: 10, fontWeight: 800, letterSpacing: 3, color: cfg.accentColor, marginBottom: 8 }}>
          ✦ BOSS VAINCU ✦
        </div>

        <div style={{ fontFamily: 'var(--f-title)', fontSize: 26, fontWeight: 900, color: '#fff', letterSpacing: 2, lineHeight: 1.1 }}>
          PALIER {palier - 1}
        </div>
        <div style={{ fontFamily: 'var(--f-ui)', fontSize: 12, color: 'var(--text-sub)', marginTop: 4, marginBottom: 18 }}>
          {prev.name} — terminé
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginBottom: 18 }}>
          {rewards.map(r => (
            <div key={r.label} style={{ flex: 1, background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 6px' }}>
              <div style={{ fontSize: 17, lineHeight: 1, marginBottom: 5 }}>{r.icon}</div>
              <div style={{ fontFamily: 'var(--f-num)', fontWeight: 800, fontSize: 15, color: r.color, lineHeight: 1 }}>{r.value}</div>
              <div style={{ fontFamily: 'var(--f-ui)', fontSize: 9, color: 'var(--text-dim)', marginTop: 3 }}>{r.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '9px 12px', borderRadius: 10, background: `${cfg.accentColor}12`, border: `1px solid ${cfg.accentColor}44`, marginBottom: 16 }}>
          <span style={{ fontFamily: 'var(--f-ui)', fontSize: 11, color: 'var(--text-dim)' }}>Prochain monde :</span>
          <span style={{ fontFamily: 'var(--f-ui)', fontSize: 11, fontWeight: 800, color: cfg.accentColor, letterSpacing: 0.5 }}>{cfg.name}</span>
        </div>

        <button onClick={onClose} className="btn-primary" style={{ width: '100%', padding: 11, fontSize: 14 }}>
          CONTINUER
        </button>
      </div>
    </div>
