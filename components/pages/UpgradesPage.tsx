      ...
      <div style={{ display:'flex', gap:12, alignItems:'flex-start', paddingLeft:8 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:4 }}>
            ...
            <span style={{ fontFamily:'var(--f-num)', fontWeight:700, fontSize:14, color:'var(--green)', flexShrink:0, marginLeft:8 }}>{formatNumber(dps)}/s</span>
          </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {active.map(syn => (
            <div key={syn.def.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 12px', background:`${syn.def.color}10`, border:`1px solid ${syn.def.color}44`, borderRadius:8, boxShadow:`0 0 10px ${syn.def.glow}15` }}>
              ...
              <div style={{ fontFamily:'var(--f-num)', fontWeight:900, fontSize:14, color:syn.def.color, flexShrink:0 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 }}>
          {[
            ...
            <div key={s.label} className="panel" style={{ padding:'16px 18px' }}>
              <div style={{ fontFamily:'var(--f-ui)', fontSize:9, fontWeight:700, color:'var(--text-dim)', letterSpacing:1.5, marginBottom:8, display:'flex', gap:4 }}><span>{s.icon}</span><span>{s.label}</span></div>
              <div style={{ fontFamily:'var(--f-num)', fontWeight:900, fontSize:22, color:s.color, lineHeight:1.05 }}>{s.val}</div>
            </div>
          ))}
        </div>
