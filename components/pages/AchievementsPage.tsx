          ...
          right={
            <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'6px', minWidth:'180px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', width:'100%' }}>
                <span style={{ fontFamily:'var(--f-ui)', fontSize:'10px', color:'var(--text-dim)', fontWeight:700 }}>PROGRESSION</span>
                <span style={{ fontFamily:'var(--f-num)', fontSize:'12px', color:'#fbbf24', fontWeight:700 }}>{pct}%</span>
              </div>
        ...
        <div style={{ display:'flex', gap:'8px' }}>
          {(['achievements','titles'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              style={{ padding:'8px 18px', borderRadius:'8px', cursor:'pointer', fontFamily:'var(--f-ui)', fontWeight:700, fontSize:'12px', letterSpacing:'0.5px', transition:'all 0.15s',
                    ...
                    <div style={{ display:'flex', alignItems:'flex-start', gap:'12px' }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:'8px', marginBottom:'3px' }}>
                          <span style={{ fontFamily:'var(--f-ui)', fontWeight:800, fontSize:'13px', color: done ? '#fbbf24' : 'var(--text)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                            {isSecret ? '???' : a.name}
                          </span>
                          {done && <span style={{ fontSize:'14px', flexShrink:0 }}>✅</span>}
                        </div>
                        <div style={{ fontFamily:'var(--f-ui)', fontSize:'11px', color:'var(--text-dim)', marginBottom:'8px', lineHeight:1.4 }}>
                        ...
                        {!done && !isSecret && (
                          <>
                            ...
                            <div style={{ fontFamily:'var(--f-num)', fontSize:'10px', color:'var(--text-dim)' }}>
                ...
                return (
                  <div key={a.id} onClick={() => isUnlk && setActiveTitle(titleStr)}
                    ...
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'6px' }}>
                      ...
                      {isActive && <span style={{ fontFamily:'var(--f-ui)', fontSize:'9px', fontWeight:700, color:'#fbbf24', letterSpacing:'1px', background:'rgba(251,191,36,0.15)', border:'1px solid rgba(251,191,36,0.3)', padding:'2px 7px', borderRadius:'4px' }}>ACTIF</span>}
                      {!isUnlk && <span style={{ fontSize:'12px' }}>🔒</span>}
                    </div>
                    <div style={{ fontFamily:'var(--f-title)', fontSize:'15px', fontWeight:700, color: isActive ? '#fbbf24' : isUnlk ? 'var(--text)' : 'var(--text-muted)', letterSpacing:'1px', marginBottom:'3px' }}>
                      « {titleStr} »
                    </div>
                    <div style={{ fontFamily:'var(--f-ui)', fontSize:'10px', color:'var(--text-dim)' }}>
                      {isUnlk ? a.name : `Succès : ${a.name}`}
                    </div>
                    <div style={{ marginTop:'6px', fontFamily:'var(--f-num)', fontSize:'10px', fontWeight:800, color: isUnlk ? 'var(--gold-hi)' : 'var(--text-muted)' }}>
