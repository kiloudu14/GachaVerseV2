  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:9990, background:'rgba(4,3,14,0.95)', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div className="panel panel--glow" style={{ width:'100%', maxWidth:480, padding:'32px 28px', display:'flex', flexDirection:'column', gap:20, textAlign:'center' }}>
        ...
        <div>
          <div style={{ fontFamily:'var(--f-title)', fontSize:22, fontWeight:900, color:'var(--purple-glow)', letterSpacing:3, marginBottom:8 }}>
            PRESTIGE {prestigeLevel + 1}
          </div>
          <div style={{ fontFamily:'var(--f-ui)', fontSize:13, color:'var(--text-dim)', lineHeight:1.7 }}>
        ...
        <div style={{ background:'rgba(248,113,113,0.06)', border:'1px solid rgba(248,113,113,0.2)', borderRadius:10, padding:'14px 18px', textAlign:'left' }}>
          <div style={{ fontFamily:'var(--f-ui)', fontSize:10, fontWeight:700, color:'#f87171', letterSpacing:2, marginBottom:8 }}>✕ RÉINITIALISÉ</div>
          {['Palier → 1', 'Pixel-Coins → 0', 'Upgrades attaque & or → 0', 'Niveau héros → 1'].map(item => (
            <div key={item} style={{ fontFamily:'var(--f-ui)', fontSize:12, color:'rgba(248,113,113,0.8)', marginBottom:3 }}>• {item}</div>
          ))}
        </div>
        ...
        <div style={{ background:'rgba(74,222,128,0.06)', border:'1px solid rgba(74,222,128,0.2)', borderRadius:10, padding:'14px 18px', textAlign:'left' }}>
          <div style={{ fontFamily:'var(--f-ui)', fontSize:10, fontWeight:700, color:'#4ade80', letterSpacing:2, marginBottom:8 }}>✓ CONSERVÉ</div>
          {['Collection gacha complète', 'Neko-Gemmes', 'BossCrowns & VoidOrbs', 'Succès & expéditions', '+3 Points de Prestige', `+${(PRESTIGE_PASSIVE_DPS_PER_LEVEL*100).toFixed(0)}% DPS & +${(PRESTIGE_PASSIVE_COIN_PER_LEVEL*100).toFixed(0)}% coins passifs`].map(item => (
            <div key={item} style={{ fontFamily:'var(--f-ui)', fontSize:12, color:'rgba(74,222,128,0.8)', marginBottom:3 }}>• {item}</div>
          ))}
        </div>

        {/* Confirmation saisie */}
        <div>
          <div style={{ fontFamily:'var(--f-ui)', fontSize:12, color:'var(--text-dim)', marginBottom:8 }}>
          ...
          <input
            ...
            style={{ width:'100%', boxSizing:'border-box', background:'rgba(255,255,255,0.04)', border:`1px solid ${ready ? 'rgba(147,51,234,0.6)' : 'var(--border)'}`, borderRadius:8, padding:'10px 14px', fontFamily:'var(--f-num)', fontSize:16, color:'var(--purple-glow)', textAlign:'center', letterSpacing:4, outline:'none', transition:'border-color 0.2s' }}
          />
        </div>

        <div style={{ display:'flex', gap:10 }}>
          <button onClick={onCancel} className="btn-secondary" style={{ flex:1, padding:'12px', fontSize:13, cursor:'pointer' }}>
            ANNULER
          </button>
          <button onClick={onConfirm} disabled={!ready} className="btn-primary"
            style={{ flex:1, padding:'12px', fontSize:13, opacity: ready ? 1 : 0.35, cursor: ready ? 'pointer' : 'not-allowed' }}>
            ⭐ CONFIRMER
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ height:'100%', overflowY:'auto', padding:'24px 28px' }}>
      ...
      <div style={{ maxWidth:900, margin:'0 auto', display:'flex', flexDirection:'column', gap:22 }}>
        ...
        <div className="panel panel--glow" style={{ padding:'22px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:20, position:'relative', overflow:'hidden' }}>
          ...
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:6 }}>
              ...
              <span style={{ fontFamily:'var(--f-title)', fontSize:18, fontWeight:700, color:'var(--purple-glow)', letterSpacing:'3px' }}>NEW GAME+</span>
              {level > 0 && (
                <div style={{ background:'rgba(147,51,234,0.2)', border:'1px solid rgba(147,51,234,0.5)', borderRadius:6, padding:'2px 10px', fontFamily:'var(--f-num)', fontWeight:900, fontSize:14, color:'#c084fc' }}>
                  {'⭐'.repeat(Math.min(level, 5))} {level > 5 ? `×${level}` : ''}
                </div>
              )}
            </div>
            <div style={{ fontFamily:'var(--f-ui)', fontSize:12, color:'var(--text-dim)', maxWidth:420, lineHeight:1.6 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:8, flexShrink:0 }}>
            {eligible ? (
              <button onClick={() => setShowConfirm(true)} className="btn-primary"
                style={{ padding:'14px 28px', fontSize:15, letterSpacing:2, display:'flex', alignItems:'center', gap:10 }}>
                ⭐ PRESTIGE {level + 1}
              </button>
            ) : (
              <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid var(--border)', borderRadius:10, padding:'12px 20px', textAlign:'center' }}>
                <div style={{ fontFamily:'var(--f-num)', fontWeight:700, fontSize:14, color:'var(--text-dim)' }}>
                  🔒 Palier {maxPalierReached} / 40
                </div>
                <div style={{ fontFamily:'var(--f-ui)', fontSize:11, color:'var(--text-muted)', marginTop:3 }}>
            ...
            <div style={{ fontFamily:'var(--f-num)', fontWeight:700, fontSize:13, color:'#fbbf24' }}>
        ...
        {/* Stats actives */}
        <div>
          <div style={{ fontFamily:'var(--f-ui)', fontWeight:700, fontSize:11, color:'var(--text-dim)', letterSpacing:2, marginBottom:12 }}>BONUS ACTIFS</div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:10 }}>
            {STAT_ROWS.map((s, i) => (
              <div key={i} className="panel" style={{ padding:'16px 18px' }}>
                <div style={{ fontFamily:'var(--f-ui)', fontSize:9, fontWeight:700, color:'var(--text-dim)', letterSpacing:1.5, marginBottom:8 }}>{s.label}</div>
                <div style={{ fontFamily:'var(--f-num)', fontWeight:900, fontSize:20, color:s.color, lineHeight:1.05 }}>{s.val}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {level > 0 && (
          <div className="panel" style={{ padding:'16px 20px', borderColor:'rgba(147,51,234,0.3)' }}>
            <div style={{ fontFamily:'var(--f-ui)', fontWeight:700, fontSize:11, color:'var(--purple-glow)', letterSpacing:2, marginBottom:10 }}>✦ BONUS PASSIFS PAR NIVEAU (×{level})</div>
            <div style={{ display:'flex', gap:16, flexWrap:'wrap' }}>
              <div style={{ fontFamily:'var(--f-ui)', fontSize:13, color:'var(--green)' }}>
                🔥 +{(level * PRESTIGE_PASSIVE_DPS_PER_LEVEL * 100).toFixed(0)}% DPS total
              </div>
              <div style={{ fontFamily:'var(--f-ui)', fontSize:13, color:'var(--gold)' }}>
        ...
        {/* Shop prestige */}
        <div>
          <div style={{ fontFamily:'var(--f-ui)', fontWeight:700, fontSize:11, color:'var(--text-dim)', letterSpacing:2, marginBottom:12 }}>
            ...
            {PRESTIGE_UPGRADES.map(upg => {
              ...
              return (
                <div key={upg.id} className="panel" style={{
                  ...
                  <div style={{ display:'flex', alignItems:'flex-start', gap:12, marginBottom:10 }}>
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:3 }}>
                        <span style={{ fontFamily:'var(--f-ui)', fontWeight:800, fontSize:13, color: maxed ? '#4ade80' : 'var(--text)' }}>
                          {upg.name}
                        </span>
                        {upg.maxLevel > 1 && (
                          <span style={{ fontFamily:'var(--f-num)', fontWeight:700, fontSize:11, color: maxed ? '#4ade80' : '#fbbf24', background: maxed ? 'rgba(74,222,128,0.1)' : 'rgba(251,191,36,0.1)', border:`1px solid ${maxed ? 'rgba(74,222,128,0.3)' : 'rgba(251,191,36,0.3)'}`, borderRadius:4, padding:'2px 7px' }}>
                            {currentLevel}/{upg.maxLevel}
                          </span>
                        )}
                      </div>
                      <div style={{ fontFamily:'var(--f-ui)', fontSize:11, color:'var(--text-dim)', lineHeight:1.5 }}>{upg.description}</div>
                    </div>
                  </div>
                  ...
                  {maxed ? (
                    <div style={{ fontFamily:'var(--f-ui)', fontSize:12, fontWeight:700, color:'#4ade80', textAlign:'center', padding:'8px', background:'rgba(74,222,128,0.06)', borderRadius:6, border:'1px solid rgba(74,222,128,0.2)' }}>
                      ✅ NIVEAU MAXIMUM
                    </div>
                  ) : (
                    <button onClick={() => buyUpgrade(upg.id)} disabled={!canBuy}
                      className={canBuy ? 'btn-primary' : 'btn-secondary'}
                      style={{ width:'100%', padding:'9px', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', gap:8, cursor: canBuy ? 'pointer' : 'not-allowed', opacity: canBuy ? 1 : 0.45 }}>
                      <span>ACHETER</span>
