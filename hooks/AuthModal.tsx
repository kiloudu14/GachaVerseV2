  ...
  if (requestSent) return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--c-panel)', border: '2px solid #fbbf24', padding: '28px', width: '340px', display: 'flex', flexDirection: 'column', gap: '12px', boxShadow: '0 0 32px #fbbf2444', textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: 'var(--font-pixel)', fontSize: '10px', color: '#fbbf24' }}>DEMANDE ENVOYÉE</div>
        <div style={{ fontFamily: 'var(--font-pixel)', fontSize: '8px', color: 'var(--c-muted)', lineHeight: 1.8 }}>
          Ton compte est créé mais doit être validé manuellement avant de pouvoir jouer.
          Ça ne devrait pas prendre longtemps !
        </div>
        <button onClick={onClose} style={{ padding: '10px', background: '#8b5cf622', border: '1px solid #8b5cf6', color: '#a78bfa', cursor: 'pointer', fontFamily: 'var(--font-pixel)', fontSize: '9px' }}>FERMER</button>
      </div>
    </div>
  );
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--c-panel)', border: '2px solid #8b5cf6', padding: '24px', width: '340px', display: 'flex', flexDirection: 'column', gap: '14px', boxShadow: '0 0 32px #8b5cf644' }}>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {(['signin','signup'] as const).map(m => (
            <button key={m} onClick={() => { setMode(m); setError(''); }} style={{ flex: 1, padding: '8px', background: mode===m ? '#8b5cf622' : 'none', border: `1px solid ${mode===m ? '#8b5cf6' : 'var(--c-border)'}`, color: mode===m ? '#a78bfa' : 'var(--c-muted)', cursor: 'pointer', fontFamily: 'var(--font-pixel)', fontSize: '8px' }}>
              {m === 'signin' ? 'CONNEXION' : 'DEMANDER'}
            </button>
          ))}
        </div>
        {mode === 'signup' && (
          <div style={{ fontFamily: 'var(--font-pixel)', fontSize: '7px', color: 'var(--c-muted)', lineHeight: 1.6, padding: '8px', background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.25)' }}>
